...
interface RequestForQuotation {
  ...
  date: string;
  requiredBy: string;
  status: 'Draft' | 'Sent' | 'Partially Responded' | 'Fully Responded' | 'Closed' | 'Cancelled';
...
export default function RequestForQuotation() {
  ...
  const [rfqs, setRFQs] = useState<RequestForQuotation[]>([
    {
      ...
      title: 'Steel Sheets Procurement',
      ...
      items: [
        { id: '1', itemCode: 'RM-001', itemName: 'Steel Sheets 2mm', quantity: 500, uom: 'NOS', description: 'Cold rolled steel sheets' },
        { id: '2', itemCode: 'RM-002', itemName: 'Aluminum Bars', quantity: 300, uom: 'KG', description: 'Aluminum 6061 bars' }
      ],
      supplierQuotes: [
        { supplierId: 'SUP-001', supplierName: 'ABC Manufacturing Co.', quoteAmount: 175000, deliveryDays: 10, status: 'Received' },
        { supplierId: 'SUP-002', supplierName: 'XYZ Electronics Ltd.', quoteAmount: 165000, deliveryDays: 15, status: 'Pending' }
      ]
    },
    {
      id: '2',
      ...
      items: [
        { id: '1', itemCode: 'EC-001', itemName: 'Resistor Pack 100k', quantity: 1000, uom: 'NOS', description: 'Surface mount resistors' },
        { id: '2', itemCode: 'EC-002', itemName: 'Capacitor 100uF', quantity: 500, uom: 'NOS', description: 'Electrolytic capacitors' }
      ],
      supplierQuotes: [
        { supplierId: 'SUP-002', supplierName: 'XYZ Electronics Ltd.', quoteAmount: 15000, deliveryDays: 7, status: 'Received' },
        { supplierId: 'SUP-003', supplierName: 'PQR Packaging Solutions', quoteAmount: 13500, deliveryDays: 10, status: 'Pending' }
      ]
    },
    {
      id: '3',
      ...
      items: [
        { id: '1', itemCode: 'PKG-001', itemName: 'Carton Boxes Large', quantity: 200, uom: 'NOS', description: 'Corrugated boxes' },
        { id: '2', itemCode: 'PKG-002', itemName: 'Packing Tape', quantity: 150, uom: 'ROL', description: 'Industrial packing tape' }
      ],
      supplierQuotes: []
    }
  ]);
  ...
  const statusOptions = ['Draft', 'Sent', 'Partially Responded', 'Fully Responded', 'Closed', 'Cancelled'];
  ...
  const filteredRFQs = rfqs.filter(r => {
    const matchesSearch = r.rfqNumber.toLowerCase().includes(filterText.toLowerCase()) ||
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'Partially Responded': return 'status-partial';
      case 'Fully Responded': return 'status-completed';
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'Sent': return <FaPaperPlane size={10} />;
      case 'Partially Responded': return <FaClock size={10} />;
      case 'Fully Responded': return <FaCheckCircle size={10} />;
  ...
  const handleEdit = (rfq: RequestForQuotation) => {
    ...
    setFormData({
      ...
      notes: rfq.notes || '',
      items: rfq.items.map(item => ({ ...item }))
    });
    setShowEditModal(true);
  };

  const handleView = (rfq: RequestForQuotation) => {
    setSelectedRFQ(rfq);
    setShowViewModal(true);
  };

  const handleDelete = (rfq: RequestForQuotation) => {
    setSelectedRFQ(rfq);
  ...
  const addItemRow = () => {
    ...
    setFormData(prev => ({
      ...prev,
      items: [...prev.items, { id: newId, itemCode: '', itemName: '', quantity: 1, uom: 'NOS', description: '' }]
    }));
  };
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      if (showEditModal && selectedRFQ) {
        setRFQs(prev => 
          prev.map(r => r.id === selectedRFQ.id 
            ? { 
                ...r, 
                ...formData,
                supplierQuotes: r.supplierQuotes,
        ...
        const newRFQ: RequestForQuotation = {
          ...
          company: formData.company || 'Manufacturing ERP',
          supplier: formData.supplier,
          supplierCode: formData.supplierCode || `SUP-${String(rfqs.length + 1).padStart(3, '0')}`,
          date: formData.date,
  ...
  const handleDuplicate = (rfq: RequestForQuotation) => {
    const newRFQ: RequestForQuotation = {
      ...rfq,
      id: String(rfqs.length + 1),
      rfqNumber: `RFQ-2026-${String(rfqs.length + 1).padStart(3, '0')}`,
  ...
  const respondedRFQs = rfqs.filter(r => r.status === 'Partially Responded' || r.status === 'Fully Responded').length;

  return (
    <div className={`rfq-page ${theme}-theme`}>
      <style>{`
        /* ── Page Container ─────────────────────────────────────── */
        .rfq-page {
          display: flex;
        ...
        .rfq-page::-webkit-scrollbar { width: 4px; }
        .rfq-page::-webkit-scrollbar-track { background: transparent; }
        .rfq-page::-webkit-scrollbar-thumb { background: var(--border-color, #e5e7eb); border-radius: 2px; }
        ...
        @media (max-width: 480px) {
          .rfq-page { padding: 8px 12px; }
          .rfq-table { font-size: 12px; min-width: 600px; }
          .rfq-table th, .rfq-table td { padding: 6px 10px; }
          .action-group { gap: 0; }
          .action-btn { width: 24px; height: 24px; }
        }

        /* ─── Dark Theme ─────────────────────────────────────────── */
        .dark-theme .rfq-page { background: var(--layout-bg, #0f172a); }
        ...
        .dark-theme .rfq-container { background: var(--card-bg, #1e293b); border-color: var(--border-color, #334155); }
        .dark-theme .rfq-table thead { background: var(--layout-bg, #0f172a); }
        .dark-theme .rfq-table th { color: var(--text-secondary, #94a3b8); border-bottom-color: var(--border-color, #334155); }
        .dark-theme .rfq-table td { border-bottom-color: var(--border-color, #334155); color: var(--text-primary, #f8fafc); }
        .dark-theme .rfq-row:hover { background: var(--nav-hover, rgba(255,255,255,0.05)); }
        .dark-theme .rfq-number-cell { color: var(--primary-color, #818cf8); }
        ...
        .dark-theme .status-sent { background: rgba(59,130,246,0.2); color: #60a5fa; }
        ...
        .dark-theme .quote-status-pending { background: rgba(107,114,128,0.2); color: #9ca3af; }
        .dark-theme .quote-status-received { background: rgba(59,130,246,0.2); color: #60a5fa; }
        .dark-theme .quote-status-accepted { background: rgba(16,185,129,0.2); color: #34d399; }
        .dark-theme .quote-status-rejected { background: rgba(239,68,68,0.2); color: #f87171; }
      `}</style>
      ...
      <div className="page-header">
        <div className="header-left">
          ...
          <span className="badge">{rfqs.length}</span>
        </div>
        <div className="header-actions">
          <button className="btn-primary" onClick={handleCreate}>
            <FaPlus size={12} /> Add RFQ
          </button>
        </div>
      </div>
      ...
      <div className="compact-stats">
        ...
        <div className="stat-item">
          ...
          <span className="stat-value stat-sent">{sentRFQs}</span>
        </div>
        <div className="stat-divider" />
        <div className="stat-item">
          <span className="stat-label">Responded</span>
          <span className="stat-value stat-responded">{respondedRFQs}</span>
        </div>
        <div className="stat-divider" />
        <div className="stat-item">
          <span className="stat-label">Draft</span>
          <span className="stat-value">{rfqs.filter(r => r.status === 'Draft').length}</span>
        </div>
      </div>
      ...
      <div className="search-bar">
        ...
        <div className="filter-wrapper">
          ...
          <span className="result-count">{filteredRFQs.length} of {rfqs.length}</span>
        </div>
      </div>
              ...
              <tbody>
                {filteredRFQs.map((rfq) => (
                  <tr key={rfq.id} className="rfq-row">
                    <td className="rfq-number-cell">{rfq.rfqNumber}</td>
                    <td className="title-cell">{rfq.title}</td>
                    <td className="supplier-cell">{rfq.supplier}</td>
                    <td className="date-cell">{new Date(rfq.date).toLocaleDateString()}</td>
                    <td className="date-cell">{new Date(rfq.requiredBy).toLocaleDateString()}</td>
                    <td>
                      <span className={`priority-badge ${getPriorityColor(rfq.priority)}`}>
                        {rfq.priority}
                      </span>
                    </td>
                    <td>
      ...
      <div className="list-footer">
        <span>{filteredRFQs.length} of {rfqs.length} RFQs</span>
        <div className="footer-actions">
          <span className="conversion-badge">
            <FaPaperPlane size={11} /> {sentRFQs} sent
          </span>
        </div>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowViewModal(false)}>
          <div className="modal-container view-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{selectedRFQ.rfqNumber} - {selectedRFQ.title}</h2>
            ...
            <div className="modal-body">
              <div className="view-grid">
                <div className="view-section">
                  ...
                  <div className="view-row"><label>RFQ #:</label><span>{selectedRFQ.rfqNumber}</span></div>
                  <div className="view-row"><label>Title:</label><span>{selectedRFQ.title}</span></div>
                  <div className="view-row"><label>Status:</label><span className={`status-badge ${getStatusColor(selectedRFQ.status)}`}>{selectedRFQ.status}</span></div>
                  <div className="view-row"><label>Priority:</label><span className={`priority-badge ${getPriorityColor(selectedRFQ.priority)}`}>{selectedRFQ.priority}</span></div>
                </div>
                <div className="view-section">
                  <h4>Supplier Details</h4>
                  <div className="view-row"><label>Supplier:</label><span>{selectedRFQ.supplier}</span></div>
                  <div className="view-row"><label>Code:</label><span>{selectedRFQ.supplierCode}</span></div>
                  <div className="view-row"><label>Company:</label><span>{selectedRFQ.company}</span></div>
                  <div className="view-row"><label>Currency:</label><span>{selectedRFQ.currency}</span></div>
                </div>
                <div className="view-section">
                  <h4>Dates</h4>
                  <div className="view-row"><label>Date:</label><span>{new Date(selectedRFQ.date).toLocaleDateString()}</span></div>
                  <div className="view-row"><label>Required By:</label><span>{new Date(selectedRFQ.requiredBy).toLocaleDateString()}</span></div>
                  <div className="view-row"><label>Created By:</label><span>{selectedRFQ.createdBy}</span></div>
                </div>
                <div className="view-section">
                  <h4>Supplier Quotes</h4>
                  {selectedRFQ.supplierQuotes.length > 0 ? (
                    selectedRFQ.supplierQuotes.map((quote, idx) => (
                      <div key={idx} className="view-row" style={{ flexDirection: 'column', alignItems: 'flex-start', gap: '2px' }}>
                        <span><strong>{quote.supplierName}</strong></span>
                        <span>Amount: {selectedRFQ.currency} {quote.quoteAmount.toLocaleString()}</span>
                        <span>Delivery: {quote.deliveryDays} days</span>
                        <span className={`quote-status-badge quote-status-${quote.status.toLowerCase()}`}>{quote.status}</span>
                      </div>
                    ))
                  ) : (
                    <div className="view-row"><span>No quotes received yet</span></div>
                  )}
                </div>
                <div className="view-section full-width">
                  ...
                  <table className="view-items-table">
                    <thead>
                      <tr><th>Item Code</th><th>Item Name</th><th>Qty</th><th>UOM</th><th>Description</th></tr>
                    </thead>
                    <tbody>
                ...
                {selectedRFQ.notes && (
                  <div className="view-section full-width">
                    <h4>Notes</h4>
                    <div className="view-row"><span>{selectedRFQ.notes}</span></div>
                  </div>
                )}
              </div>
            </div>
            <div className="modal-footer">
              ...
              <button className="btn-primary" onClick={() => handleEdit(selectedRFQ)}>
                <FaEdit size={12} /> Edit
              </button>
              <button className="btn-primary" onClick={() => toast.success('RFQ sent to supplier!')}>
      ...
      {(showCreateModal || showEditModal) && (
        <div className="modal-overlay" onClick={() => {
          ...
          <div className="modal-container" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{showEditModal ? 'Edit RFQ' : 'New Request for Quotation'}</h2>
        ...
        <div className="modal-overlay" onClick={() => setShowDeleteModal(false)}>
          <div className="modal-container delete-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body delete-body">
              ...
              <h3>Are you sure?</h3>
              <p>
                You are about to delete <strong>{selectedRFQ.rfqNumber}</strong>
              </p>
